...
def main(rank: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
  traindir = args["traindir"]
  ...
  def get_weight_decay(raw_model, lr_scale, warmup_scale, train_state, running_metrics, group_name):
    if raw_model.get_norm_kind() == "fixup" or raw_model.get_norm_kind() == "fixscale":
      if group_name == "normal" or group_name == "normal_gamma" or group_name == "output":
        return 0.000001 * batch_size / 256.0
      ...
      if group_name == "normal" or group_name == "normal_gamma":
        ...
        return 0.00125 * batch_size / 256.0 * math.pow(lr_scale * warmup_scale,0.75) * adaptive_scale * gamma_scale
  ...
  def load():
    ...
    if path_to_load_from is None:
      ...
      metrics_obj = Metrics(batch_size,raw_model)
      running_metrics = {}
      train_state = {}
  ...
  while True:
    ...
    for i in range(sub_epochs):
      ...
      for batch in data_processing_pytorch.read_npz_training_data(
        ...
        #else:
        model_outputs = ddp_model(batch["inputLocalNCHW"],batch["inputGlobalNC"])
    ...
    if rank == 0:
      ...
      if not no_export and is_time_to_export and not skip_export_this_time and exportdir is not None and not gnorm_stats_debug:
        ...
        if os.path.exists(savepath):
          ...
          dummy_input = (torch.randn(256, 21, 9, 9, device="cuda:0"), torch.randn(256, 11, device="cuda:0"))
          torch.onnx.export(ddp_model, dummy_input, os.path.join(savepathtmp,"model.onnx"), input_names=["input_local", "input_global"], output_names=["output_policy", "output_value", "output_land"])
          time.sleep(2)
...
if __name__ == "__main__":
  ...
  if num_gpus_used > 1:
    ...
    #world_size = 1
    barrier = None
    main(rank, args, multi_gpu_device_ids, readpipes, writepipes, barrier)
