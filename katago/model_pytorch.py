...
#class Conv2dSamePadding(torch.nn.Conv2d):
    #def __init__(self,*args,**kwargs):
        ...
        #self.zero_pad_2d = torch.nn.ZeroPad2d(reduce(__add__,
...
class KataConvAndGPool(torch.nn.Module):
    def __init__(self, c_in, c_out, c_gpool, config, activation):
        ...
        self.conv1r = torch.nn.Conv2d(c_in, c_out, kernel_size=3, padding=1, bias=False)
        self.conv1g = torch.nn.Conv2d(c_in, c_gpool, kernel_size=3, padding=1, bias=False)
        self.normg = NormMask(
            c_gpool,
            config=config,
...
class KataConvAndAttentionPool(torch.nn.Module):
    def __init__(self, c_in, c_out, c_gpool, config, activation):
        ...
        self.conv1r = torch.nn.Conv2d(c_in, c_out, kernel_size=3, padding=1, bias=False)
        self.conv1g = torch.nn.Conv2d(c_in, c_gpool, kernel_size=3, padding=1, bias=False)
        self.conv1k = torch.nn.Conv2d(c_in, c_gpool, kernel_size=1, bias=False)
        self.conv1q = torch.nn.Conv2d(c_in, c_gpool, kernel_size=1, bias=False)
        ...
        self.conv_mix = torch.nn.Conv2d(c_gpool*2, c_out, kernel_size=1, bias=False)
...
class NormActConv(torch.nn.Module):
    def __init__(
        ...
        if c_gpool is not None:
            ...
            if kernel_size > 1:
                self.conv = torch.nn.Conv2d(c_in, c_out, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=False)
            else:
                self.conv = torch.nn.Conv2d(c_in, c_out, kernel_size=kernel_size, bias=False)
            self.convpool = None

        self.conv1x1 = None
        if self.conv is not None and kernel_size > 1 and "use_repvgg_linear" in config and config["use_repvgg_linear"]:
            self.conv1x1 = torch.nn.Conv2d(c_in, c_out, kernel_size=1, bias=False)
...
class PolicyHead(torch.nn.Module):
    def __init__(self, c_in, c_p1, c_g1, config, activation):
        ...
        self.conv1p = torch.nn.Conv2d(c_in, c_p1, kernel_size=1, bias=False)
        self.conv1g = torch.nn.Conv2d(c_in, c_g1, kernel_size=1, bias=False)
        ...
        self.linear_pass = torch.nn.Linear(3 * c_g1, 1, bias=False)
        ...
        self.conv2p = torch.nn.Conv2d(c_p1, self.num_policy_outputs, kernel_size=1, bias=False)
    ...
    def forward(self, x, mask, mask_sum_hw, mask_sum:float):
        ...
        outpass = self.linear_pass(outg) # N1
        outg = self.linear_g(outg).unsqueeze(-1).unsqueeze(-1) # NC11 (?)
        ...
        # N(CHW) concat with N1
        return torch.cat((outpolicy.view(outpolicy.shape[0],-1), outpass),dim=1)
...
class ValueHead(torch.nn.Module):
    def __init__(self, c_in, c_v1, c_v2, c_sv2, config, activation, pos_len):
        ...
        self.linear_miscvaluehead = torch.nn.Linear(c_v2, 8, bias=True)
        self.conv_ownership = torch.nn.Conv2d(c_v1, 1, kernel_size=1, bias=False)
...
class Model(torch.nn.Module):
    def __init__(self, config: modelconfigs.ModelConfig, pos_len: int):
        ...
        if config["initial_conv_1x1"]:
            self.conv_spatial = torch.nn.Conv2d(21, self.c_trunk, kernel_size=1, bias=False)
        else:
            self.conv_spatial = torch.nn.Conv2d(21, self.c_trunk, kernel_size=3, padding=1, bias=False)
        self.linear_global = torch.nn.Linear(11, self.c_trunk, bias=False)

        self.bin_input_shape = [21, pos_len, pos_len]
        self.global_input_shape = [11]
    ...
    #def postprocess_single_heads_output(self, outputs):
        ...
        #pred_tiny = out_miscvalue
        #ownership_pretanh = out_ownership

        #return (
            #policy_logits,      # N, y * x * 8 + 1
            #pred_tiny,          # N, 8
            #ownership_pretanh,  # N, y * x
        #)
